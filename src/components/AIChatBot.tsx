  ...
  const handleSend = async (text: string, isPresetClick = false) => {
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        console.warn("Failed checking dynamic database FAQs:", err);
      }
    }
              ...
              <div className="px-4 pb-2 pt-1 flex flex-wrap gap-1.5 border-t border-white/5 bg-black/10">
                {activeSuggestions.map((suggestion) => (
                  <button
                    ...
                    disabled={isLoading}
                    className="text-[11px] bg-white/5 hover:bg-purple-600/20 hover:border-purple-500/30 text-zinc-300 hover:text-white border border-white/5 rounded-lg px-2.5 py-1 text-left transition-all duration-200 disabled:opacity-50"
                  >
                    {suggestion}
                  </button>
                ))}
